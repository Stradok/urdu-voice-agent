...
**Real appointment booking with business hours — ✅ live (2026-08-09).** Originally found live (2026-08-08): asked the dental-clinic account to book "tomorrow" and it agreed without checking that tomorrow was a Sunday. Investigating properly (rather than patching the symptom) surfaced two root causes, both bigger than first assumed:
1. **The system prompt never told the LLM today's date or day-of-week at all** (confirmed by reading `src/persona.py`/`src/llm.py` directly) - "tomorrow" was being resolved with zero grounding. `ChatEngine.reply()` now injects a date/day-name system message computed via `zoneinfo.ZoneInfo(AppSettings.timezone)` (never bare `datetime.now()` - Render, the deploy target, runs UTC, and naive server-local time would silently reintroduce the same bug near midnight PKT).
2. **There was no real booking tool at all.** The original note misattributed this to `TableSlot` - that model is actually for the unrelated restaurant `book_table` tool. The dental-clinic mechanism was `ServiceItem.available_slots`, a flat manually-seeded string array with no hours concept, and - the more serious finding - **no tool ever wrote a booking anywhere**. `check_appointment_slots` only read that list; when Sara "confirmed" an appointment, nothing was persisted. It was a purely hallucinated confirmation.

Fixed with a real scheduling system: `BusinessHours` (always exactly 7 rows per business, explicit `is_closed` rather than "missing row = closed" - a forgotten row should be a detectable "not configured" state, not silently misread as closed) drives a generated `AppointmentSlot` grid (`src/scheduling.py::ensure_slots_generated()`, generate-on-read for a rolling 14-day window via a watermark on `AppSettings.slots_generated_until` - no cron/worker exists in this project, and Render's free tier doesn't cleanly support one). Slots are business-wide, not per-service (no doctor/staff entity exists in the schema - one shared calendar matches a solo-clinic reality; a service longer than the grid unit consumes multiple contiguous slots). A new `book_appointment` tool (`src/tools.py`) row-locks (`SELECT ... FOR UPDATE`) the exact candidate slots, validates they're really open/contiguous/in the future, and only then persists a real `Appointment` row and flips the slots to `"booked"` - it never trusts a model-generated time, only a `slot_start` string copied verbatim from what `check_appointment_slots` actually offered.

Bright Smile Dental Clinic's hours were set to Mon-Sat 10:00-20:00, Sunday closed - matching its own seeded FAQ answer, which was previously just unenforced prose. **Verified live end-to-end**, on a day that was actually a real Sunday during testing: asked for an appointment "today" - Sara correctly declined and offered real Monday slots instead of confirming; picking one of those slots and providing a name produced a real persisted `Appointment` row (`AP-85732`, correct customer/service/time) with the matching `AppointmentSlot` flipped to booked; a second attempt to book the same slot was correctly rejected with real remaining alternatives (the just-booked time no longer listed).

Also added a reference/ticket-code system shared between appointments and bank escalations (`src/reference_codes.py`) - short, digit-only codes (`AP-#####` / `ES-#####`, easy to read aloud over voice without O/0 or I/1 ambiguity), business-scoped uniqueness, collision-safe via a `SAVEPOINT`-isolated retry (not a plain transaction rollback, which would have wiped out unrelated work like the slot-locking already done earlier in the same `book_appointment` transaction - caught and fixed during implementation, not assumed correct). `recommend_human_agent`'s escalation and `book_appointment`'s confirmation both now return a real code with an instruction to read it back one digit at a time. Verified live: a bank escalation returned and would speak back a real `ES-#####` code, landing correctly in `Escalation.reference_code`.

Deliberately out of scope for this pass: Google Calendar sync and a WhatsApp channel (both raised in the same conversation) need external OAuth/API app registration that hasn't been done yet - see the sections above for what each requires. A dedicated Appointments admin page is a reasonable fast-follow (there's currently no dashboard visibility into bookings beyond querying the DB) but wasn't built, per explicit scope - the backend model is cheap to expose later.
