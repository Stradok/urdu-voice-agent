...
- **Tool-calling for live data**: stock levels, menu items, and table bookings are looked up (and, for bookings, updated) live via `src/tools.py` — the LLM decides when to call these and grounds its reply in the actual result instead of guessing. Dental-clinic appointments go through a real business-hours-aware scheduler (`src/scheduling.py`): `check_appointment_slots` only ever offers times a generated, hours-respecting grid actually has open, and `book_appointment` row-locks and validates the exact chosen slot before persisting anything — it never trusts a model-guessed time. A `recommend_human_agent` tool lets Sara hand off sensitive cases to a real person, logged for the Continuous Learning page. Both `book_appointment` and `recommend_human_agent` return a short, voice-friendly reference code (`src/reference_codes.py`) that Sara reads back to the customer.
...
| `src/tools.py` | Stock/appointment/menu/booking/escalation functions (Postgres, `business_id`-scoped), their tool schemas, and `BUSINESS_TYPE_TOOLS`/`tool_schemas_for()` (which tools a given business type is even offered) |
| `src/scheduling.py` | Real business-hours-aware appointment scheduling — `BusinessHours`, generate-on-read `AppointmentSlot` grid, `find_available_starts()`, `now_local()` (never bare `datetime.now()`) |
| `src/reference_codes.py` | Short, voice-friendly reference/ticket codes (`AP-#####`/`ES-#####`) for appointments and escalations, collision-safe via a SAVEPOINT-isolated retry |
| `src/tts.py` | Text-to-speech via Azure Speech (SSML + rate tuning, retries once on failure), plays audio |
| `src/session_log.py` | Session/exchange/escalation logging (Postgres), returns a reference code from `log_escalation()`, read by the Continuous Learning page |
